...
/*   Updated: 2016/05/03 11:56:48 by jbyttner         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float		goursat(vec3 p, vec3 mods)
{
	...
	p2 = pow(p, vec3(2));
	return (-(sum(pow(p, VEC3(4))) + mods.x * pow(sum(p2), 2) + mods.y * sum(p2) + mods.z));
}
...
s_res		goursat_dst(s_geo sp, s_cam cam, s_res prev)
{
	...
	if (ret.dst >= 0)
	{
		...
		ret.cam = cam;
		ret.mat = sp.mat;
		ret.normal = normalize(-goursat_grad((p.xyz - sp.pos) * 5 / sp.bounds, sp.a.xyz));
		return (ret);
	}
	return (prev);
}
